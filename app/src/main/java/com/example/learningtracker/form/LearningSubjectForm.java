...
public class LearningSubjectForm implements Serializable {
    ...
    @Pattern(regexp="[a-zA-Z0-9一-龠ぁ-んァ-ヶ+&@#/%?=~_|!:,.;]+", message="項目名はひらがな、カタカナ、漢字、半角の英数字と記号で入力してください")
	private String name;

    @Size(max = 255, message = "説明は500文字以内で入力してください")
    @Pattern(regexp="[a-zA-Z0-9一-龠ぁ-んァ-ヶ+&@#/%?=~_|!:,.;]+", message="説明はひらがな、カタカナ、漢字、半角の英数字と記号で入力してください")
